...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for key in corpus.keys():
        pageranks[key] = [0] * len(corpus)

    random_page = random.choice(corpus.keys())

    for _ in range(n - 1):
        pageranks[random_page] += 1
        probability_distribution = transition_model(
            corpus, random_page, damping_factor)
        random_page = random.choices(population=list(probability_distribution.keys(
        )), weights=list(probability_distribution.values()))[0]

    return {page: num_samples/n for page, num_samples in pageranks.items()}
    ...
    while any(pagerank_change > 0.001 for pagerank_change in pagerank_changes.values()):
        for page in pageranks.keys():
            ...
            new_pagerank = ((1 - damping_factor) / total_pages) + \
                (damping_factor * link_probability)
