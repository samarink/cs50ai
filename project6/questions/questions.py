...
def tokenize(document):
    ...
    return [word for word in words if (word not in string.punctuation) and (word not in stopwords)]
...
def compute_idfs(documents):
    ...
    idfs = {}
    words = set(word for doc in documents.values() for word in doc)
...
def top_files(query, files, idfs, n):
    ...
    for file, words in files.items():
        ...
        for word in query:
            idf += words.count(word) * idfs[word]
...
def top_sentences(query, sentences, idfs, n):
    ...
    for sentence, words in sentences.items():
        ...
        scores[sentence] = {'idf': idf, 'qtd': query_term_density}

    ranked_sentences = sorted(scores.items(), key=lambda x: (
