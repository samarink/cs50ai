...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap:
            ...
            for word in self.domains[x]:
                char = word[overlap[0]]
                y_chars = {w[overlap[1]] for w in self.domains[y]}

                if char not in y_chars:
                    inconsistent_words.add(word)
                    revised = True
    ...
    def ac3(self, arcs=None):
        ...
        if arcs:
            queue = list(itertools.product(
                self.crossword.variables, self.crossword.variables))
    ...
    def order_domain_values(self, var, assignment):
        ...
        sorted_list = sorted(
            num_choices_eliminated.items(), key=lambda x: x[1])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        num_remaining_values = {variable: len(
            self.domains[variable]) for variable in unassigned_variables}

        sorted_num_remaining_values = sorted(
            num_remaining_values.items(), key=lambda x: x[1])
        ...
        if len(sorted_num_remaining_values) == 1 or sorted_num_remaining_values[0][1] != sorted_num_remaining_values[1][1]:
            ...
            num_degrees = {variable: len(self.crossword.neighbors(
                variable)) for variable in unassigned_variables}
            sorted_num_degrees = sorted(
                num_degrees.items(), key=lambda x: x[1], reverse=True)
